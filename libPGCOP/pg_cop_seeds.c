...
int pg_cop_seed_destroy(pg_cop_seed_t *seed)
{
	...
	free(seed->mod_name);
	seed->mod_name = NULL;
	pg_cop_module_interface_destroy(seed->intf);
	free(seed);
...
{
	free(tracker->host);
	pg_cop_module_interface_destroy(tracker->intf);
	return 0;
}
